...
\begin{enumerate}[label=(\alph*), left=0pt, labelsep=1em, itemsep=1em]
    \item 
    According to notations defined in writeup:
    ...
    2. $\mathbf y$ is an one-hot vector where only $y_o=1$.
    \qed
    ...
    1. $\hat{\mathbf y} = \mathbf y$ (impossible due to positive nature of softmax), or
    ...
    then gradient descent updates $\mathbf v_c$ by pulling it towards the true context vector $\mathbf U\mathbf y$ and repelling it from the (false) predicted distribution $\mathbf U\hat{\mathbf y}$.
\end{enumerate}
